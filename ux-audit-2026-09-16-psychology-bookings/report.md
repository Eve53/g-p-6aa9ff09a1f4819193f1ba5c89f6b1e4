...
页面已经把“今天的预约”置于列表首位，并提供醒目的“进入咨询室”入口，核心任务具备可完成性。主要风险不是缺少功能，而是页面的信息架构更像“按咨询师罗列”，与用户想确认“我的订单现在处于什么状态、下一步做什么”的心智模型不完全一致。最需要优先处理的是：将导航和列表改为状态/任务导向、为即将开始的咨询提供明确时间反馈与进入规则、为“审核中”补齐等待预期和后续路径。截图只能证明静态呈现；按钮反馈、禁用规则、提醒机制、异常处理等均作为建议，不假设已经存在。
...
- **证据边界：** 只将截图中的可见内容作为现状证据。动态状态、点击热区、加载与延迟、消息通知、进入时限、错误反馈、取消或改期流程、焦点顺序、键盘和屏幕阅读器表现无法从截图判断。

## 问题总览

| ID | 优先级 | 检查项 | 问题摘要 | 主要依据 |
|---|---|---|---|---|
| F-01 | S3 高 | CTA-AMBIGUITY | 顶部导航按“咨询师/咨询室”组织，与订单状态任务不匹配 | Nielsen #2/#6；Norman: Conceptual model；Tog: Visible navigation |
...
### F-01｜导航分类不贴合“看订单状态”的用户任务

- **当前问题 ➡️** 顶部一级切换为“全部咨询师 / 我的咨询室”，但当前内容实际混合展示“今天有预约、暂无预约、审核中”等订单/服务状态。“全部咨询师”也容易被理解为咨询师市场，而非我的订单列表。
- **用户影响 ➡️** 首次用户需要先推断“订单在哪里”，回访用户也难以按待办紧迫度快速定位；“我的咨询室”与卡片内“进入咨询室”概念接近，却处于不同导航层级，增加概念混淆。
- **推荐交互方案 ➡️** 将一级导航改为任务导向，例如“我的咨询 / 咨询师”，并在“我的咨询”内使用“待进行、待预约、审核中、已完成”筛选；默认聚焦最近待办。若保留现结构，应补充清晰页面标题或说明，明确“全部咨询师”展示的是已购买服务的咨询师。
- **优先级 ➡️ S3 高。** 直接影响两个核心目标的入口识别。依据：Nielsen #2、#6；Norman: Conceptual model；Tog: Visible navigation。
...
当前路径为“识别顶部分类 → 逐卡扫描视觉差异 → 从日期/颜色/文案推断状态 → 必要时进入详情”。主要摩擦来自导航概念与任务不一致、状态组件不统一、“审核中”缺少下一步。建议路径为“进入我的咨询 → 默认看到按紧迫度排序的待办 → 通过统一状态标签立即识别 → 一键执行当前状态对应动作”，历史订单通过筛选直接访问。
...
1. **高优先级：** 将页面导航与筛选改为订单状态/待办导向，统一卡片结构与状态标签。
2. **高优先级：** 为最近预约补齐倒计时、进入开放时间、设备检查、加载与失败恢复。
3. **高优先级：** 将“审核中”改为可预期流程：预计时长、是否需要操作、通知方式、超时求助。
4. **中优先级：** 收敛每卡操作数量，历史记录上移；统一合同、预约记录等次级入口。
5. **中低优先级：** 验证对比度、触控面积和客服悬浮层在多机型/大字体下的表现。

## 框架覆盖

| 框架 | 相关发现 |
|---|---|
| Nielsen | F-01、F-02、F-03、F-04、F-05、F-07、F-09 |
| Shneiderman | F-02 |
